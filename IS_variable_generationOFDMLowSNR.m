function [  theta_est_circular, Delay_est_conditioned, tau_est_circular_corrected]=IS_variable_generationOFDMLowSNR(g_prime_bar,theta_vector_points,step_theta,tau_vector_points,~,Q,R,~, ~,noise_variance,rho_g,rho_Lc,M,S_time,N_FFT,H,wc,delta_f,rol,c)
...
for hh=1:Q
    ...
    tau_vector_points=[tau_vector_points max_points(1,hh)-1.4*1e-8:step_tau*1e-8:max_points(1,hh)+1.4*1e-8];
...
[g_prime_bar, S_frequency, X_hat]=Importance_function_gridOFDM(wc,H,S_time,theta_vector_pointsrad,step_theta,tau_vector_points,step_tau,P,N_FFT,N,noise_variance,rho_g,delta_f,rol,c);
 ...
 %-------------------------------------
%---------local pdf
 size_avoiding_interval_tau=ceil(1.4/step_tau);
...
size_avoiding_interval_tau=ceil(1.3/step_tau);
