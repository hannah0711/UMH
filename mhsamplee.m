function [smpl,accept,x1,x2] = mhsamplee(start,nsamples,varargin)
...
for i = 1-burnin:nsamples*thin
    ...
    %%%%%Avoid negative delay and set the range for the doa%%%%%
    if (length(start) == 2)
    while (y(2) < 0 || y(1) < -90 || y(1) >90 )
        y = proprnd(x0);
    end
    end
    
    if (length(start) == 4)
    while (y(3) < 0 || y(4) < 0 || y(1) < -90 || y(1) >90 || y(2) < -90 || y(2) >90 )
        y = proprnd(x0);
    end
    end
    
    if (length(start) == 6)
    while (y(4) < 0 || y(5) < 0 || y(6) < 0 || y(1) < -90 || y(1) >90 || y(2) < -90 || y(2) >90 || y(3) < -90 || y(3) >90 )
        y = proprnd(x0);
    end
    end
